...
def available_rooms():
    ...
    try:
        ...
        # Check if both start_date and end_date are in the past
        if start_datetime < current_datetime and end_datetime < current_datetime:
            return jsonify({'success': False, 'message': 'Both start date and end date are in the past. Please enter valid dates.'}), 400
        
        # Proceed with fetching available rooms if dates are valid
        ...
        # Handle ValueError if the date format is incorrect
        return jsonify({'success': False, 'message': 'Invalid date format. Please use the format YYYY-MM-DD.'}), 400
